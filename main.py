...
# parser.add_argument('john', help='this is a john!')
...
""" this block generates training, testing ATB tuples, calcualates surprisal w/GPT2, and plots it. This data is saved in atb_testing.json, atb_training.json
# generate train, test atb sentence tuples
atb_training = []
atb_testing = []
for i, text_grammar in enumerate(grammars.ATB_GRAMMARS):
    grammar = CFG.fromstring(text_grammar)
    training, testing = gc.generate_train_test_sentence_tuples_from_grammar(
        grammar=grammar,
        split_ratio=0.65)
...
# calculate surprisal on atb testing set
...
gc.corpus_to_json(atb_training, "atb_training.json")
gc.corpus_to_json(atb_testing, "atb_testing.json")
"""


atb_training = gc.corpus_from_json(
